...
- **비용 통제** — 한국어 검색·표기는 외부 기계번역 없이 정적 사전 매핑(번역 API 비용 0 + 직급 오역 회피), LLM 요약은 공고당 1회 호출 후 DB 캐시, LLM 재분류는 옵트인.
...
- **맞춤 추천** — 로그인 홈에서 프로필 5축 매칭 + pgvector 의미 검색으로 추천(별도 페이지 없이 홈에 통합), 점수 근거 공개, 다양성 제약(회사 편중 방지)
...
| 맞춤 추천 — 로그인 홈 (프로필 기반 5축 매칭) | AI 이력서 코치 |
|---|---|
