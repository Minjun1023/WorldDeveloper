...
// 상세 상단 전폭 정보바: 지원/저장 + 핵심 메타(가로) + 회사/비자가이드.
    ...
    <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex w-full gap-2 sm:w-auto">
          {job.apply_url ? (
            <a
              ...
              className="flex h-11 flex-1 items-center justify-center gap-1.5 rounded-[10px] bg-primary px-6 text-body-sm font-bold text-primary-foreground transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:flex-none"
            >
            ...
            <div
              aria-disabled="true"
              className="flex h-11 flex-1 items-center justify-center rounded-[10px] bg-surface-2 px-6 text-body-sm font-semibold text-muted-foreground sm:flex-none"
            >
              지원 링크 미제공
            </div>
          )}
        ...
        <Link
          ...
          className="flex items-center gap-2.5 rounded-xl border border-border p-2 pr-3 transition-colors hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
          ...
          <div className="min-w-0">
            ...
            {companyJobCount !== undefined && companyJobCount > 1 && (
              <div className="text-caption text-muted-foreground">공고 {companyJobCount}개</div>
            )}
          </div>
        </Link>
      </div>
      ...
      {/* 메타 가로 그리드 */}
      <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 border-t border-border pt-4 text-body-sm sm:grid-cols-3 lg:grid-cols-4">
        {rows.map((r) => (
      ...
      {guide && (
        <Link
          ...
          className="mt-4 block rounded-xl border border-border p-3 text-body-sm transition-colors hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
