...
// 다이제스트 메일의 "그만 받기" 링크 랜딩 — 브라우저 GET 이므로 사람이 읽을 HTML 을 돌려준다.
// 토큰만으로 동작(로그인 불필요): 메일 수신자가 곧 구독자라는 전제의 표준 원클릭 해지 패턴.
export async function GET(req: Request) {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/api/v1/alerts/unsubscribe?token=${encodeURIComponent(token)}`,
      { cache: "no-store", signal: AbortSignal.timeout(5000) },
    );
    if (res.ok) {
      ok = true;
      label = ((await res.json()) as { label?: string }).label ?? "";
    }
  } catch {
    // ok=false 로 안내
  }
