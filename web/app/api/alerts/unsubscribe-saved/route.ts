...
// 저장 공고 마감 알림 메일의 "그만 받기" 링크 랜딩 — alerts/unsubscribe 와 동일 패턴.
export async function GET(req: Request) {
  const token = new URL(req.url).searchParams.get("token") ?? "";
  let ok = false;
  try {
    const res = await fetch(
      `${BACKEND_URL}/api/v1/alerts/unsubscribe-saved?token=${encodeURIComponent(token)}`,
  ...
  const body = ok
    ? `<h1>알림이 해지되었습니다</h1><p>저장 공고 마감 이메일 알림을 더 이상 보내지 않아요.</p><p><a href="/bookmarks">북마크로 가기</a></p>`
