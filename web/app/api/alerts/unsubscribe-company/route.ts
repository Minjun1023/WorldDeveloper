...
// 관심 기업 다이제스트 메일의 "그만 받기" 링크 랜딩 — alerts/unsubscribe 와 동일 패턴.
// 토큰만으로 동작(로그인 불필요): 메일 수신자가 곧 구독자라는 전제의 표준 원클릭 해지.
export async function GET(req: Request) {
  const token = new URL(req.url).searchParams.get("token") ?? "";
  let ok = false;
  try {
    const res = await fetch(
      `${BACKEND_URL}/api/v1/alerts/unsubscribe-company?token=${encodeURIComponent(token)}`,
  ...
  const body = ok
    ? `<h1>알림이 해지되었습니다</h1><p>관심 기업 새 공고 이메일 알림을 더 이상 보내지 않아요.</p><p><a href="/bookmarks?tab=companies">관심 기업으로 가기</a></p>`
