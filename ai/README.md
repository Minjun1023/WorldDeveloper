...
| `DEEPL_API_KEY` | (빈값) | 번역(DeepL). 무료 키는 `:fx` 로 끝남. 빈 값이면 번역 503. 온디맨드+캐시(`job_translations`)라 같은 공고는 1회만 번역 |
| `ETL_TRANSLATE_BACKFILL` | `false` | 야간 ETL에서 미번역 전량 백필 여부. DeepL 무료 한도(월 100만자) 초과 위험으로 기본 off(온디맨드 권장) |
